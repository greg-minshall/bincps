...
real file is: ./ipums/cps_00006.csv.gz
#+name: binthem
#+BEGIN_SRC R :session ss :var fname="./ipums/19712014.csv.gz" :var maxk=500
  require(ggplot2)
  x <- gzfile(fname, open="r")
  dset <- read.csv(x, header=TRUE)

  # first, describe the negative incomes
  report <- rbind(summary(dset[dset$HHINCOME<0,]$HHINCOME))
  rrnames <- c("<0");
  # now, kill them
  ...
  dset <- cbind(dset, BIN=.bincode(dset$HHINCOME1999,
                        seq(from=0, to=maxk, by=1),
                        right=FALSE));

  # HHINCOME1999 greater than, or equal to, MAXK is labeled NA by
  # .bincode
  dset[is.na(dset$BIN)] <- max(dset$BIN, na.rm=TRUE)+1

  # and, describe these
  report <- rbind(report, summary(dset[dset$BIN == maxk+1,]))
  rrnames <- c(rrnames, sprintf(">%d", maxk))

  rownames(report) <- rrnames

  # (we don't describe *other* bins since they are of limited range; the
  # "negative" and "greater than max" bins are not of an a priori known
  # limit.)
  ...
  for (year in unique(dset$YEAR)) {
    s <- dset$YEAR == year
    for (bin in unique(dset[s,]$BIN)) {
      if (!is.na(bin)) {
        s <- s & dset$BIN == bin
        brnames <- c(brnames, sprintf("%d", i));
      } else {
        x <- s & is.na(dset$BIN)
        brnames <- c(brnames, sprintf(">%d", maxk));
        bin <- max(dset$BIN, na.rm=TRUE)+1
      }
      for (asecflag in unique(dset[s,]$ASECFLAG)) {
        s <- s & dset$ASECFLAG == asecflag
        for (hflag in unique(dset[s,]$HFLAG)) {
          s <- s & dset$HFLAG == hflag
                                          # *finally* -- do something!
          month <- unique(dset[s,]$MONTH);
          if (length(month) > 1) {
            month <- NA
          }
          best <- rbind(bset, c(YEAR=year,
                                HWTSUPP=sum(dset[s,]$HWTSUPP),
                                ASECFLAG=asecflag,
                                HFLAG=hflag,
                                UPTOHHINCOME1999=bin*1000,
                                MONTH=month,
                                WTSUPP=sum(dset[s,]$WTSUPP)));
        }
      }
    }
  }
