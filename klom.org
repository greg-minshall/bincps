...
#+BEGIN_SRC R :session ss :var ifile="./ipums/cps_00006.csv.gz" :var ofile="" :var ofsep="-" :var rfile="" :var fyear="-" :var lyear="+" :var min1999="-" :var max1999="+"
  ...
  bincps1 <- function(ifile,             # input file
                      dset,              # data set
    ...
    ahroutine <- function(filter, upto) {
      if (FALSE) {
        print(sprintf("ahroutine, year %d, nrow filter %d, upto %g, nrow bset %d",
                      year, nrow(yset[filter,]), upto, nrow(bset)))
      }
...
#+BEGIN_SRC R
  summary.binned <- function(m) {
    ...
    result["Min"] <- m["value"][1]             # minimum value
    result["Max"] <- m["value"][nrow(m)]       # maximum value
    ...
    ## now the order statistics
    m <- cbind(m, cumq=cumsum(m["quantity"])) # get cumulative sum
    ## so we can use findInterval
    i <- findInterval(c(population/4, population/2, 3*population/4), m["cumq"])
    if ((i < 1) || (i > nrow(m))) {
      stop("internval error")             # pun intended
    }
    result["1st Qu."] <- m["value"][i[1]]
    result["Median"] <- m["value"][i[2]]
    result["3rd Qu."] <- m["value"][i[3]]
...
#+BEGIN_SRC R :session ss
  ...
  # cumulative sums of [H]WTSUPP (relies on being ordered)
  data <- cbind(data, CUMWTSUPP=0, CUMPCTWTSUPP=0, CUMHWTSUPP=0, CUMPCTHWTSUPP=0)
  # https://stackoverflow.com/a/32487458 on computing cumpct
