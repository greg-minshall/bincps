...
real file is: ./ipums/cps_00006.csv.gz
#+name: bincps
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var maxk=500 :var ofile="" :var ofsep="-"
  bincps <- function(ifile, maxk=500, ofile="", ofsep="-") {
    ...
    if (ofile == "") {
      ...
      x[length(x)-1] <- sprintf("binned%s%dK.%s", ofsep, maxk, x[length(x)-1]);
      ofile <- paste(x, collapse=".")
    }

    if (file.exists(ofile)) {
      stop(sprintf("output file \"%s\" exists, won't remove", ofile))
    }
    ...
    s <- dset$HHINCOME < 0
    report <- rbind(c(NROW=nrow(dset[s,]), summary(dset[s,]$HHINCOME)))
    rrnames <- c("<0")
                                          # now, kill them
    dset <- dset[dset$HHINCOME >= 0,]

                                          # now, get all the bins...
    dset <- cbind(dset, BIN=.bincode(dset$HHINCOME1999/1000,
                          seq(from=0, to=maxk, by=1),
                          right=FALSE));

                                          # HHINCOME1999 greater than,
                                          # or equal to, MAXK is labeled
                                          # NA by .bincode
    maxkbin <- max(dset$BIN, na.rm=TRUE)+1
    dset[is.na(dset$BIN),]$BIN <- maxkbin

                                          # and, describe these
    s <- dset$BIN == maxk+1
    report <- rbind(report, c(NROW=nrow(dset[s,]),
                              summary(dset[dset$BIN == maxk+1,]$HHINCOME1999)))
    rrnames <- c(rrnames, sprintf(">%d", maxk))

    rownames(report) <- rrnames
    ...
    for (year in sort(unique(dset$YEAR))) {
      ...
      for (bin in sort(unique(dset[sy,]$BIN))) {
        sb <- sy & dset$BIN == bin
        if (bin != maxkbin) {
          brname <- sprintf("%d", bin)
          upto <- bin*1000;
        } else {
          brname <- sprintf(">%d", maxk)
