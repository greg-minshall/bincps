...
#+name: bincps
#+BEGIN_SRC R :session ss :var ifile="./ipums/cps_00006.1962-2017-binned.csv" :var ofile="foo.csv" :var ofsep="-" :var rfile="goo.csv" :var fyear="-" :var lyear="+" :var min1999="-" :var max1999="+"
  ...
  bincps <- function(ifile,              # input file
    ...
    rval <- bincps1(ifile, dset, ...)
    bset <- rval[[1]]
    rset <- rval[[2]]
    write.csv(bset, ofile, row.names=F);
    if (nrow(rset) != 0) {
      write.csv(rset, rfile, row.names=F)
    }
  }
  ...
  print("")
  bincps(ifile=ifile, dset, ofile=ofile, rfile=rfile, ofsep=ofsep, fyear=fyear, lyear=lyear, min1999=min1999, max1999=max1999);
#+END_SRC
