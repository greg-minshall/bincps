...
#+BEGIN_SRC R :session ss
  ...
  data <- read.csv("~/work/misc/econ/kblom/ipums/cps_00006.1962-2017-binned.csv")

  data$YEAR <- as.factor(data$YEAR)

  ## binsize?  to figure this out, we look at the smallest difference
  ## between successive UPTOHHINCOME99
  x <- data$UPTOHHINCOME99
  y <- unique(c(x[2,length(x)], NA) - x)
  binsize <- min(y, na.rm=TRUE)

  MAX99 <- 500000
  MIN99 <- 0

  datalow <- data[data$UPTOHHINCOME99 < MIN99,]
  datahigh <- data[data$UPTOHHINCOME99 > MAX99,]
  data <- data[data$UPTOHHINCOME99 >= MIN99 & data$UPTOHHINCOME99 <= MAX99,]

  ## XXX should make sure we are using MIN99+1000
  ...
  ggplot(data, aes(UPTOHHINCOME99, colour = YEAR)) + stat_ecdf()
