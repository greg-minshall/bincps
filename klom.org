...
#+BEGIN_SRC R :session ss :var fname="./ipums/19712014.csv.gz" :var maxk=500
  ...
  report <- rbind("<0"=summary(dset[dset$HHINCOME<0,]$HHINCOME))
  # now, kill them
  ...
  dset <- cbind(dset, BIN=.bincode(dset$HHINCOME1999, seq(from=0, to=maxk, by=1), right=FALSE));

  # HHINCOME1999 greater than, or equal to, MAXK is labeled NA.
  dset[is.na(dset$BIN)] <- max(dset$BIN, na.rm=TRUE)+1

  # and, describe these
  report <- rbind(report, sprintf("\">%sk", maxk)=summary(dset[dset$BIN == maxk+1,]))

  # (we don't describe *other* bins since they are of limited range; the
  # negative and greater than max bins are not of a priori known limit.)
