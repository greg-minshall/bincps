...
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var ofile="" :var ofsep="-" :var rfile="" :var fyear="-" :var lyear="+" :var min1999="0" :var max1999="500000"
  ...
  bincps <- function(ifile,              # input file
    ...
    ## first, make min, max multiples of binsize
    fromvalue <- floor(min(dset$HHINCOME1999)/binsize)*binsize
    tovalue <- ceiling(max(dset$HHINCOME1999)/binsize)*binsize
    dset <- cbind(dset, BIN=.bincode(dset$HHINCOME1999,
                          seq(from=fromvalue, to=tovalue, by=binsize),
                          right=right));
