...
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var maxk=500 :var ofile="" :var ofsep="-" :var rfile="" :var fyear=-Inf :var lyear=+Inf
  ...
  bincps <- function(ifile,               # input file
                     min1999=0,           # minimum HHINCOME1999 (in USD)
                     max1999=500000,      # maximum HHINCOME1999 (in USD)
    ...
    ofile <- dealwithoutputfilename(ifile, ofile, "output",
                                    sprintf("%sbinned", ofset));
  
    rfile <- dealwithoutputfilename(ifile, rfile, "report",
                                    sprintf("%sbinned%sreport", ofset, ofset))
