...
#+name: bincps
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var ofile="" :var ofsep="-" :var rfile="" :var fyear="2014" :var lyear="+" :var min1999="0" :var max1999="500000"
      ...
      for (asecflag in unique(dset[filter,]$ASECFLAG)) {
        ...
        for (hflag in unique(dset[sa,]$HFLAG)) {
          ...
          if (nrow(dset[sh,]) != 0) {
            ...
            if (nrow(bset) == 0) {
              bset <<- rbind(      # https://stackoverflow.com/a/5231670
                             c(YEAR=year,
