...
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var ofile="" :var ofsep="-" :var rfile="" :var fyear="-" :var lyear="+"
  ...
  bincps <- function(ifile,               # input file
    ...
    dset <- cbind(dset[sy,], BIN=.bincode(dset$HHINCOME1999/binsize,
    ...
    for (year in sort(unique(dset$YEAR))) {
      ...
      ## <<noweb>> working in org-mode when running code via C-c C-c).
      ## this routine is called to enter rows into the output table.
      ahroutine <- function(upto) {
      ...
      slow <- sy & slowbit
      if (nrow(dset[slow,]) != 0) {
        rset <- rbind(c(NROW=nrow(dset[slow,]),
      ...
      /* now, deal with too high data
      if (right) {
        shighbit <- dset$HHINCOME1999 >= max1999
        comment <- sprintf("greater than, or equal to, %02f", min1999);
      } else {
        shighbit <- dset$HHINCOME1999 > max1999
        comment <- sprintf("greater than %02f", min1999);
      }
      shigh <- sy & shighbit
      if (nrow(dset[shigh,]) != 0) {
        rset <- rbind(c(NROW=nrow(dset[shigh,]),
  ...
  bincps(ifile=ifile, ofile=ofile, rfile=rfile, ofsep=ofsep, fyear=fyear, lyear=lyear);
#+END_SRC
