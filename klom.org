...
#+name: bincps
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var maxk=500 :var ofile="" :var ofsep="-" :var rfile="" :var fyear=-Inf :var lyear=+Inf
  bincps <- function(ifile, maxk=500, ofile="", ofsep="-") {
    ...
    if (ofile == "") {
      ...
      x[length(x)] <- sprintf("binned%s%dK.%s", ofsep, maxk, x[length(x)]);
      ofile <- paste(x, collapse=".")
    }

    if (file.exists(ofile)) {
      stop(sprintf("output file \"%s\" exists, won't overwrite", ofile))
    }
    ...
    if (rfile == "") {
      ...
      x <- strsplit(ifile, ".", fixed=TRUE)[[1]]
      if (x[length(x)] == "gz") {
        length(x) = length(x)-1           # again, get rid of .gz extension
      }
      x[length(x)] <- sprintf("binned%sreport.%s", ofsep, x[length(x)])
      rfile <- paste(x, collapse=".")
    }

    if (file.exists(rfile)) {
      stop(sprintf("report file \"%s\" exists, won't overwrite", rfile))
    }
    ...
    dset <- cbind(dset[sy,], BIN=.bincode(dset$HHINCOME1999/1000,
                               seq(from=min(dset$HHINCOME1999)/1000,
                                   to=max(dset$HHINCOME1999)/1000, by=1),
                               right=FALSE));

    bset <- data.frame()
    for (year in sort(unique(dset$YEAR))) {
      if ((year < fyear) | (year > lyear)) {
        next
      }
      sy <- dset$YEAR == year
                                          # describe the negative incomes
      sy1 <- sy & dset$HHINCOME1999 < 0
      rset <- rbind(c(NROW=nrow(dset[sy1,]),
                      summary(dset[sy1,]$HHINCOME1999)
                      COMMENT="less than zero"))
                                          # now, kill them
      sy <- sy & dset$HHINCOME1999 >= 0

                                          # HHINCOME1999 greater than,
                                          # or equal to, MAXK is labeled
                                          # NA by .bincode
      maxkbin <- max(dset[sy,]$BIN, na.rm=TRUE)+1
      dset[is.na(dset$BIN),]$BIN <- maxkbin

                                          # and, describe these
      s <- dset$HHINCOME1999 == maxk+1
      rset <- rbind(rset, c(NROW=nrow(dset[s,]),
                            summary(dset[dset$BIN == maxk+1,]$HHINCOME1999)
                            COMMENT=sprintf("greater than %dK", maxk)))
 
                                          # (we don't describe *other*
                                          # bins since they are of
                                          # limited range; the
                                          # "negative" and "greater than
                                          # max" bins are not of an a
                                          # priori known limit.)
      ...
      for (bin in sort(unique(dset[sy,]$BIN))) {
        ...
        if (bin != maxkbin) {
          ...
          upto <- max(dset[sb,]$HHINCOME1999)
        }
        for (asecflag in unique(dset[sb,]$ASECFLAG)) {
          if (!is.na(asecflag)) {
            sa <- sb & dset$ASECFLAG == asecflag
          } else {
            sa <- sb & is.na(dset$ASECFLAG)
          }
    ...
    rownames(rset) <- rrnames
    write.csv(bset, ofile);
  }

  bincps(ifile, maxk, ofile, ofsep);
#+END_SRC
