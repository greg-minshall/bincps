...
#+name: bincps
#+BEGIN_SRC R :session ss :var ifile="./ipums/19712014.csv.gz" :var ofile="" :var ofsep="-" :var rfile="" :var fyear="-" :var lyear="+" :var min1999="-" :var max1999="+"
  ...
  bincps <- function(ifile,              # input file
    ...
    for (year in sort(unique(dset$YEAR))) {
      ...
      for (bin in sort(unique(dset[sy,]$RANGE))) {
        sb <- sy & dset$RANGE == bin
        ahroutine(sb, bin)
      }
