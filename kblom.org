...
#+BEGIN_SRC R :session ss :results none
  ...
  binsize <- 1000                         # XXX
  LOWESTYEAR <- 1967                      # before 1967, NA data

  tidyup <- function(nvals, nobs, year) {
    "take the input dataset, cap it"
    ## now, clamp upper end at MAXHH (200,000, say)
    ## first, propagate counts of the higher incomes to end of chart
    highest <- max(nvals[nvals <= MAXHH], na.rm=TRUE)
    nobs[nvals==highest] <- nobs[nvals==highest] + sum(nobs[nvals>highest])
    ## now, delete the higher values
    nobs <- nobs[nvals >= 0 & nvals <= MAXHH]
    nvals <- nvals[nvals >= 0 & nvals <= MAXHH]
    ## get rid of never-observed values, if any (efficiency)
    nvals <- nvals[nobs!=0]
    nobs <- nobs[nobs!=0]
    ## data frame it
    df <- data.frame(year=as.factor(year), vals=nvals, nobs=nobs)
  }
  ...
  for (year in unique(dset$YEAR)) {
    yset <- dset[dset$YEAR==year,]
    tidy <- tidyup(yset[,"HHBRACKET99"], yset[,supp], year)
    my.ecdf[[year]] <- ecdf.binned(tidy$vals, tidy$nobs)
    my.iecdf[[year]] <- iecdf.binned(tidy$vals, tidy$nobs)
  }
  ...
  repro.chart <- function(dset,
    ...
    df <- tidyup(nvals=rebinvals.binned(min(yset$HHBRACKET99, na.rm=TRUE),
                   max(yset$HHBRACKET99, na.rm=TRUE), binsize),
                 nobs=rebin.binned(vals=yset$HHBRACKET99, nobs=yset$HWTSUPP,
                   newvals=newvals),
                 year=hist)

    ## now, work on 1971
    yset <- dset[dset$YEAR==line,]
    df2 <- tidyup(nvals=rebinvals.binned(min(yset$HHBRACKET99, na.rm=TRUE),
                   max(yset$HHBRACKET99, na.rm=TRUE), binsize),
                 nobs=rebin.binned(vals=yset$HHBRACKET99, nobs=yset$HWTSUPP,
                   newvals=newvals),
                 year=line)
    ...
    ## (approx) zero
    sbzero <- sum((df[df$year==hist,"nobs"]/tobs)
                  (df[df$year==line,"nobs"]/tobs))
    if (abs(sbzero) > 0.001) {
