...
#+caption: (Click on the image to see the original.)
#+name: fig:ftdisplay
...
function ([[CDF]]) to really understand what was going on.  Using data
...
(especially for a so-called "empirical cumulative distribution
function", i.e., where one is looking not at the theoretical
distribution, but rather the results of a number of experiments) is to
form a histogram, "binning" the possible values (so that all the
values from, e.g., 1-10 go in one bin, from 11-20 in another, etc.),
and then assigning each bin a "probability" based on the number of
counts in that bin divided by the total number of counts over all the
bins.  This histogram can then be displayed, looking something like
the rose-colored bars in the Financial Times [[fig:ftdisplay][display]] Blom posted.  In
the FT display, in 2015, a household income of $50,000 has about a
four percent chance of occurring.
...
But, there's another way of looking at a distribution, the "cumulative
distribution function" (CDF).  The CDF can never go down -- it can
only go up (or stay the same).  It is always in the range between zero
and one; it starts at zero at negative infinity (say), and will have
