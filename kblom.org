...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none :eval no-export :exports none
  ...
  ecdf.binned.common <- function(x, nobs, inverse, from, count) {
    ...
    if (!inverse) {
      ...
      yleft <- 0
      yright <- 1
      if (count) {                          # we want cumsum, not cumpct
          Y <- wet$ecdf*sum(nobs)
      } else {
          Y <- wet$ecdf
      }
  ...
  ecdf.binned <- function(x, nobs, count) {
    "like ecdf, but for binned values; cribbed from ecdf"
    ecdf.binned.common(x, nobs, inverse = FALSE, "ecdf.binned", count=count)
  }

  iecdf.binned <- function(x, nobs, count) {
  ...
  probably \"slight\" overkill for that usage"
    ecdf.binned.common(x, nobs, inverse = TRUE, "iecdf.binned", count=count)
  }
...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  cumcdf <- function(cumcdf) {
      "for each cumcdf, return 1: explanation; 2: label"
      switch(cumcdf,
          "population:bracket" = c("cumulative percentage of population in, or below, a given bracket", "population", "bracket"),
          "totalincome:bracket" = c("cumulative percent of total income in, or below, a given bracket", "totalincome", "bracket"),
          "totalincome:population" = c("cumulative percentage of total income in, or below, a given percentile of the population", "totalincome", "population"),
          stop(sprintf("invalid cumcdf \"%s\"", cumcdf)))
  }
  ...
  looking.create.ecdf <- function(dset, year, cumcdf, count) {
      ## create ECDF
      sy <- dset$YEAR==year
      bracket <- dset[sy, attr(dset, "bracket")]
      supp <- dset[sy, attr(dset, "supp")]
      rval <- switch(cumcdf,
             "population:bracket" = ecdf.binned(bracket, supp, count),
             "totalincome:bracket" = ecdf.binned(bracket, bracket*supp, count),
             ## in the following, we divide by "sum(supp)" to
             ## normalize across years (for comparability)
             "totalincome:population" = ecdf.binned(cumsum(supp), bracket*supp, count),
            stop(sprintf("invalid cumcdf \"%s\"", cumcdf)))
  }

  looking.create.iecdf <- function(dset, year, cumcdf, count) {
    ## create inverse (IECDF)
      sy <- dset$YEAR==year
      bracket <- dset[sy, attr(dset, "bracket")]
      supp <- dset[sy, attr(dset, "supp")]
      switch(cumcdf,
             "population:bracket" = iecdf.binned(bracket, supp, count),
             "totalincome:bracket" = iecdf.binned(bracket, bracket*supp, count),
             "totalincome:population" = iecdf.binned(cumsum(supp), bracket*supp, count))
  }
  ...
  doparser <- function(name, need, args) {
    ...
    if ("values" %in% need) {
      need <- subset(need, need!="values")
      p <- add_argument(p, "--values", type="character", default="20000",
                        help="values to check for")
    }
    if ("cumcdf" %in% need) {
      need <- subset(need, need!="cumcdf")
      p <- add_argument(p, "--cumcdf", type="character", default="population:bracket",
                        help="cumcdf of what?  \"bracket/supp\", \"supp/bracket*supp\", \"bracket/bracket*supp\"")
    }
    if ("delta" %in% need) {
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  ggp.ecdf <- function(dset, years, cumcdf, delta) {
    ds <- switch(cumcdf, 
                 "bracket/supp" = list("CDF showing what percent of population is in each household income bracket in the United States (1999 dollars)",
                   attr(dset, "maxhh"),
                   attr(dset, "bracket")),
                 "bracket/bracket*supp" = list("CDF showing what percent of total household income is accounted for by each income bracket in the United States (1999 dollars)",
                   attr(dset, "maxhh"),
                   attr(dset, "bracket")),
                 "supp/bracket*supp" = list("CDF showing what percentage of population accounts for how much of total national income",
                 1,
                 "fraction of total population"),
                 stop(sprintf("unknown \"--cumcdf\" argument \"%s\"", cumcdf)))
    title <- ds[[3]]
    if (delta) {
    ...
    if (delta) {
        ...
        maxy <- max(years)
        miny <- min(years)
        max.ecdf <- looking.create.ecdf(dset, maxy, cumcdf, count=FALSE)
        min.ecdf <- looking.create.ecdf(dset, miny, cumcdf, count=FALSE)
        base <- base + stat_function(fun=function(x) {
            max.ecdf(x)-min.ecdf(y)
        })
    } else {
        for (year in years) {
            ecdf <- looking.create.ecdf(dset, year=year, cumcdf=cumcdf, count=FALSE)
  ...
  main <- function(args=NULL) {
    ...
    ggp.ecdf(dset, years=attr(dset, "years"),
             cumcdf=attr(dset, "cumcdf"), delta=attr(dset, "delta"))
    endup(dset)
    return(0)
  }
...
#+BEGIN_SRC R :tangle create.repro :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  main <- function(args=NULL) {
    ...
    repro.chart(dset, bracket=attr(dset, "bracket"), supp=attr(dset, "supp"), years=attr(dset, "years"), cumcdf=attr(dset, "cumcdf"))
    endup(dset)
    return(0)
  }
...
this returns 
#+BEGIN_SRC R :tangle values.ecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>

  values.ecdf <- function(dset, years, vals) {
      cumcdf <- attributes(dset)$cumcdf
      df <- data.frame()
      for (year in years) {
          frac.ecdf <- looking.create.ecdf(dset, year=year, cumcdf=cumcdf, count=FALSE)
          count.ecdf <- looking.create.ecdf(dset, year=year, cumcdf=cumcdf, count=TRUE)
          yset <- dset[dset$YEAR==year,]
          for (val in vals) {
              df <- rbind(df, data.frame(year=year, value=val, count=count.ecdf(val), cumpct=frac.ecdf(val)))
          }
      }
      # ccdfex <- cumcdf(cumcdf)
  ...
  main <- function(args=NULL) {
      dset <- doparser(name="values.ecdf",
                       need=c("ifile",
                              "maxhh", "supp", "bracket",
                              "binsize",
                              "cumcdf",
                              "years", "values"),
                       args)
      df <- values.ecdf(dset, years=attr(dset, "years"), vals=attr(dset, "values"))
      endup(dset)
  }
...
this returns 
#+BEGIN_SRC R :tangle values.iecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>

  values.iecdf <- function(dset, years, tiles) {
    cumcdf <- attributes(dset)$cumcdf
    df <- data.frame()
    for (year in years) {
      frac.iecdf <- looking.create.iecdf(dset, year=year, cumcdf=cumcdf, count=FALSE)
      count.iecdf <- looking.create.iecdf(dset, year=year, cumcdf=cumcdf, count=TRUE)
      yset <- dset[dset$YEAR==year,]
      for (tile in tiles) {
          df <- rbind(df, data.frame(year=year, cumpct=tile, # XXX cumsum=dfsflk,
                                     value=count.iecdf(tile), percentile=frac.iecdf(tile)))
      }
    }
    ccdfex <- cumcdf(cumcdf)
    # colnames(df) <- c("year", ccdfex[2], ccdfex[3])
    print(df, row.names=FALSE)
  }


  main <- function(args=NULL) {
    dset <- doparser(name="values.iecdf",
                     need=c("ifile",
                       "maxhh", "supp", "bracket",
                       "binsize", "cumcdf",
                       "years", "tiles"),
                     args)
    df <- values.iecdf(dset, years=attr(dset, "years"), tiles=attr(dset, "tiles"))
    endup(dset)
  }
