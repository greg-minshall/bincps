...
outcome can be zero for *every* specific outcome (point), but still,
the *integral* over all the possible outcomes will be one; i.e.,
*some* outcome will occur.  However, in a continuous distribution, it
*can* be the case that one or more of the points (again, presumably at
...
#+RESULTS:
[[file:graphs/cdfonedimen.IMG]]

If the probability of any of these points occurring is equal to that
of any other point, this is a very reasonable display.  Normally,
though, the probability of different points will differ, in which
case, when displaying a discrete probability distribution with only a
small number of possible values, it is common to show a barchart,
where the height of each bar relates to the probability of that value
occuring.

#+BEGIN_SRC R :results graphics :exports results :file graphs/cdfbarplot50.IMG
x <- runif(50)*10 + rnorm(50,mean=5)
x <- x/ceiling(max(x))*10
y <- dnorm(x, mean=5)
y <- y/sum(y)
...
#+BEGIN_SRC R :results graphics :exports results :file graphs/cdflineplot50.IMG
...
#+BEGIN_SRC R :results graphics :exports results :file graphs/cdfwidely.IMG
  x <- sort(rnorm(50,mean=5))
  doit <- function(a,b) {
      ifelse(b > 0.5, a, .1*a)
  }
  z <- doit(dnorm(x,mean=5),rnorm(50))
  z <- z/sum(z)                           # normalize
  plot(x,z,type="l")
#+END_SRC
...
possible values (so that all the values from, e.g., 0-9 are counted
in one bin, from 10-19 counted in another, etc.), and then assigning
...
hist(x, breaks=seq(0,max(x)+10, 10))
#+END_SRC

#+RESULTS:
[[file:graphs/cdfhisto.IMG]]
...
distributions ("[[https://en.wikipedia.org/wiki/Probability_density_function][probability density function]]", p.d.f.), and displaying
the results of an experiment, i.e., drawing samples from a population
assumed to be distributed in some manner and examining the results
("[[https://en.wikipedia.org/wiki/Frequency_distribution][frequency distribution]]").  The latter case, i.e., confronted with a
data set listing a set of outcomes, looks very similar to a discrete
distribution, even in the case where the underlying distribution from
which the samples are drawn is continuous.

For a continuous distribution, the p.d.f., which something like the
...
to" zero, i.e., get thinner and thinner, approaching a width of
zero.[fn::At the same time, increasing the number of samples,
approaching an infinite number of points] In the FT display, in 1971,
a household income of about $10,000 had about a four percent chance of
occurring (as did a household income of about $75,000 in that same
year).

The plot of the probability density function, or of its histogram, is
useful, showing, as it does, places where the likelihood is (locally)
...
describes the results of an actual series of experiments, a long
series of "draws" from the distribution in question.  While the theory
of statistics, in particular [[https://en.wikipedia.org/wiki/Law_of_large_numbers][the law of large numbers]], guarantees that
...
it is also the case that, as [[https://en.wikiquote.org/wiki/John_Maynard_Keynes][Keynes said]],
#+BEGIN_QUOTE
But this long run is a misleading guide to current affairs.  In the
long run we are all dead.
#+END_QUOTE
And, as it is often the case, looking at real world "experiments", we
aren't even certain of which theoretical distribution, with which
parameters, we are looking at, it is often more valuable to think in
terms of empirical distributions.

In addition to the p.m.f. XXX (or, for continuous distributions, the
"[[https://en.wikipedia.org/wiki/Probability_density_function][probability density function]]", p.d.f.), there's another way of
looking at, or characterizing, a distribution, the "[[https://en.wikipedia.org/wiki/Cumulative_distribution_function][cumulative
distribution function]]" (CDF).  The CDF can never go down -- it can
only go up (or stay the same; we say the function is "[[https://en.wikipedia.org/wiki/Monotonic_function][monotonic]]", but
not "strictly monotonic").  It is always in the range between zero and
one; for a distribution of a single variable, the CDF starts at zero
at negative infinity (typically, at the left margin of the display)
and will have reached one by the time it arrives at positive infinity
(typically, by the right margin of the display).  The value of the
cumulative distribution function at a given point is the probability
that the a sample taken from the distribution will be less than or
equal to that point.  Oof -- what does that mean?
...
XXX show a normal bell curve, as a p.d.f. ??? and as a CDF

#+BEGIN_SRC R :results graphics :exports results :file graphs/bellpdfcdf.IMG
x <- sort(rnorm(1000))
x <- seq(-5, 5, .01)
y <- dnorm(x)
plot(x, cumsum(y)/sum(y), xlab="", ylab="", type="l", lty=2)
lines(x,y, type="l", lty=1)
legend("topleft", c("p.d.f", "CDF"), lty=c(1,2), title = "normal distribution",
            inset = .05)
#+END_SRC

#+RESULTS:
[[file:graphs/bellpdfcdf.IMG]]
...
#+name: chessboard
#+begin_src R :results graphics :file chessboard.png
