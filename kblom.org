...
Here we notice that the top 10% (from 0.9 to 1) has increased its
share of the total US household income from 25% in 1972 to almost 33%
in 2016 while, at the same time, the bottom 10% (from 0 to 0.1) has
seen *its* share of the total income decrease by about one half.  This
is to the point I saw in Blom's question, i.e, even *if* the *share*
of the bottom income bracket fell, do we really care, assuming (which
turns out to be an untrue assumption) that the actual household income
of that bottom income bracket in fact increased.
...
#+BEGIN_SRC sh :tangle create.tidistrbybracket :shebang "#!/bin/sh" :results none :exports none :eval never :var years=years
  ./create.ecdf.common $* --years ${years} --tgt.in.key %totalincome:bracket --maxhh 2e14
#+END_SRC
...
#+name: lucr
#+BEGIN_SRC R :exports none :var tgtinkey="totalincome:%population" :var keys="1" :var years="1972,2016" :var ifile=ifile :var words="FALSE" :var docomprss="TRUE" :var format="" :var diff=1 :var round=4
  <<lookup>>
  <<doprint>>
...
moved into brackets which *have*, have seen their 2016 income increase
from its previous 1972 value, then we would think of a different set
of social issues than if, rather, certain classes within our society
find themselves overly represented in the bottom ten percent without
any movement. XXX
...
#+BEGIN_SRC R :results none :eval no-export :exports none
  ...
  doparser <- function(name, need, args) {
    ...
    if ("ifile" %in% need) {
      ...
      p <- add_argument(p, "--ifile", type="character", default=system("make bins", intern=TRUE),
    ...
    if ("years" %in% need) {
      ...
      p <- add_argument(p, "--years", type="character", default=system("make years", intern=TRUE),
                        help="years to graph")
    }
    if ("tiles" %in% need) {
