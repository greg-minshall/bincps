...
#+BEGIN_SRC R :session ss :var ifile="./ipums/cps_00006.1962-2017-binned.csv" :var ofile="foo.csv" :var ofsep="-" :var rfile="goo.csv" :var fyear="-" :var lyear="+" :var min1999="-" :var max1999="+"
  bincps <- function(ifile,               # input file
    ...
    ofto <- ofsep
    if (min1999 != -Inf) {
      orlabel <- sprintf("%s%s%s", orlabel, ofsep, min1999);
      ofto <- sprintf("%sto%s", ofsep, ofsep)
    }
    if (max1999 != Inf) {
      orlabel <- sprintf("%sto%s%s%s", orlabel, ofsep, max1999);
      ofto <- ofsep
    }
    ...
    rval <- bincps1(ifile=ifile, dset=dset, ...)
    bset <- rval[[1]]
    rset <- rval[[2]]
  ...
  bincps1 <- function(                    # inherits locals from caller
                      ) {
