...
#+name: mostreported
#+BEGIN_SRC R :tangle bin/mostreported :shebang "#!/usr/bin/env Rscript" :results none :eval no-export :exports none
  <<reported.bits>>
  ...
  reported.main <- function(args=NULL) {
      dset <- doparser(name="reported",
                       need=c("reported",
                              ...
                              "years"),
                       args)
      res <- mostreported(dset, years=attr(dset, "years"))
      endup(dset)
      res
  }

  main <- function(args=NULL) {
      print(reported.main(args), row.names=FALSE)
  }
