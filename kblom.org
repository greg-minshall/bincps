...
#+begin_src R :exports none
  ...
  cboarddraw <- function(labels, background=NULL, ...) {
  ...
  rankboard <- function(labels, palette=NULL, title="", ...) {
      ...
      cb <- cboarddraw(labels, background, ...)
...
#+BEGIN_SRC R :tangle bin/create.mostreported :shebang "#!/usr/bin/env Rscript" :results none :eval no-export :exports none
  ...
  reported.main <- function(args=NULL, ...) {
      ...
      rankboard(t(res[[1]]), title="Most reported incomes for selected years", ...)
      endup(dset)
      res
  }
