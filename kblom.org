...
between the two web sites above, maybe these are the topcodes
|--------+----------+-------------|
|  99998 |  9999998 | Missing     |
|  99999 |  9999999 | N.I.U.      |
|  -9997 | -9999997 | Bottom Code |

#+name: topcodealpha
#+begin_src R :exports none :results none
  x <- t(matrix(list(99998, "Missing",
                     9999998, "Missing",
                     99999, "N.I.U.",
                     9999999, "N.I.U.",
                     -9997, "< -9999",
  ...
  doparser <- function(name, need, args) {
    ...
    if ("silentshrink" %in% need) {
      ...
      p <- add_argument(p, "--silentshrink", type="boolean", flag=TRUE,
                        help="shrink font, but do *not* warn, if labels in rankboard are too big to fit")
    }
    if ("keep" %in% need) {
...
#+begin_src R :exports none
  ...
  cboarddraw <- function(labels, background=NULL, silentshrink, ...) {
  ...
  (horizontal, vertical margins, border, padding, etc.).  this
  returns the cell size, and the (lower left) X and Y offsets
  (in pixels)"
      ...
      if (any(lsize > cellpx)) {
          if (!silentshrink) {
              warning(sprintf("at least one label is too large (%s) for box size (%d) -- shrinking font"), paste(lsize, collapse=","), cellpx)
          }
          cex <- min(.9*(cellpx/lsize))   # okay, hope this works!
      }
  ...
  rankboard <- function(labels, palette=NULL, title="", silentshrink, ...) {
      ...
      cb <- cboarddraw(labels, background, silentshrink, ...)
      justxs <- (((1:ncol(labels))-1)*cb["cellpx"])+cb["xoffpx"]+(cb["cellpx"]/2)
      ## do title, axis tick marks
      title(main=title, ...)
