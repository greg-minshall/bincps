...
#+BEGIN_SRC R :session ss :results none
  ...
  looking.create.ecdfs <- function(dset, total=FALSE) {
    ## create ECDF
    my.ecdfs <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      ...
      if (total) {
        ## want to get CDF of total bracket income over bracket
        my.ecdfs[[year]] <- ecdf.binned(bracket, supp*bracket)
      } else {
        ## else, CDF of people over bracket
        my.ecdfs[[year]] <- ecdf.binned(bracket, supp)
      }
    }
    my.ecdfs
  }

  looking.create.iecdfs <- function(dset, total=FALSE) {
    ## create inverse (IECDF)
    my.iecdfs <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      product <- dset[sy, attr(dset, "bracket")]
      if (total) {
        ## want inverse CDF of total national income
        product <- product * dset[sy, attr(dset, "supp")]
      }
      my.iecdfs[[year]] <- iecdf.binned(product, dset[sy, attr(dset, "supp")])
    }
    my.iecdfs
  }
  ...
  doparser <- function(name, need, args) {
    ...
    if ("totalincome" %in% need) {
      need <- subset(need, need!="totalincome")
      p <- add_argument(p, "--totalincome", flag=TRUE,
                        help="work on total income within each income bracket")
    }
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  ggp.ecdf <- function(dset, years=c(1973, 2015), total) {
    my.ecdfs <- looking.create.ecdfs(dset, total)
    if (total) {
      title <- ggtitle("Cumulative Distribution Function (CDF) showing what percent of total household income is accounted for by each income bracket in the United States (1999 dollars)")
    } else {
      title <- ggtitle("Cumulative Distribution Function (CDF) showing what percent of population is in each household income bracket in the United States (1999 dollars)")
    }
  ...
  main <- function(args=NULL) {
    dset <- doparser(name="looking",
                     ...
                     args)
    ggp.ecdf(dset, years=attr(dset, "years"), total=attr(dset, "totalincome"))
    endup(dset)
    return(0)
  }
...
#+BEGIN_SRC R :tangle create.repro :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  main <- function(args=NULL) {
    ...
    repro.chart(dset, bracket=attr(dset, "bracket"), supp=attr(dset, "supp"), years=attr(dset, "years"), total=attr(dset, "totalincome"))
    endup(dset)
    return(0)
  }
