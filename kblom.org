...
#+begin_src R :exports none
  ...
  rankboard <- function(labels, # matrix: text to go in boxes
                        ...
                        partition=NULL, # matrix: partition of boxes by color
                        title="", # title
  ...
  if the PARTITION matrix is null, it receives its value from the
  LABELS matrix.

  PARTITION's values will be used to compute an \"index\" into an
  array of colors for the boxes.  i.e., each unique value of
  PARTITION receives its own color, and that color is used for
  each box of the rankboard with that value of PARTITION.

  the vector PALETTE, if specified, must be at least as long as
  the number of unique values in PARTITION.
  ...
  indexing into PALETTE is done by computing the index in a *sorted*
  version of the unique values of PARTITION (or LABELS, if PARTITION
  is null).  i.e., if PARTITION[1,1] has value \"this and that\",
  and in the sorted list of unique values from PARTITION,
  \"this and that\" is the third element, then the top left box of
  the resulting graph will have the color specified by the third
  element of PALETTE."
      ...
      uni <- sort(unique(as.vector(partition))) # number of unique labels
      if (is.null(palette)) {
      ...
      background <- matrix(palette[match(partition, uni)], nrow=nrow(partition))
