...
#+BEGIN_SRC R :session ss :results none
  ...
  buildparser <- function(name, need) {
    ...
    if ("graphopts" %in% need) {
      need <- subset(need, need!="graphopts")
      p <- add_argument(p, "--graphopts", type="character", default=NULL,
  ...
  dograph <- function(graphics, graphopts) {
  ...
  undograph <- function(graphics, graphopts) {
    if (grepl("^x11$", graphics, ignore.case=TRUE)) {
...
#+BEGIN_SRC R :tangle looking.replot :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  main <- function(args=NULL) {
    ...
    dograph(argv$graphics, argv$graphopts)
    repro.chart(dset, bracket=argv$bracket, supp=argv$supp)
    undograph(argv$graphics, argv$graphopts)
    return(0)
  }
