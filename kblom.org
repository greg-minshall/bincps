...
#+BEGIN_SRC R :session ss :results none
  ...
  looking.create.ecdfs <- function(dset, cumcdf="bracket/supp") {
    ## create ECDF
    my.ecdfs <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      ...
      my.ecdfs[[year]] <-
        switch(cumcdf,
               ...
               "supp/bracket*supp" = ecdf.binned(supp, bracket*supp))
    }
    my.ecdfs
  }

  looking.create.iecdfs <- function(dset, cumcdf="bracket/supp") {
    ...
    for (year in unique(dset$YEAR)) {
      ...
      my.iecdfs[[year]] <-
        switch(cumcdf,
               ...
               "supp/bracket*supp" = iecdf.binned(supp, bracket*supp))
    }
    my.iecdfs
  }
  ...
  doparser <- function(name, need, args) {
    ...
    if ("tiles" %in% need) {
      ...
      p <- add_argument(p, "--tiles", type="character",
    ...
    if ("cumcdf" %in% need) {
      ...
      p <- add_argument(p, "--cumcdf", type="character", default="bracket/supp",
                        help="cumcdf of what?  \"bracket/supp\", \"supp/supp*bracket\", \"bracket/supp*bracket\"")
    }
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  ggp.ecdf <- function(dset, years=c(1973, 2015), cumcdf) {
    my.ecdfs <- looking.create.ecdfs(dset, cumcdf)
    ds <- switch(cumcdf, list(
                              list("CDF showing what percent of population is in each household income bracket in the United States (1999 dollars)",
                                   attr(dset, "maxhh"),
                                   attr(dset, "bracket")),
                              list("CDF showing what percent of total household income is accounted for by each income bracket in the United States (1999 dollars)",
                                   attr(dset, "maxhh"),
                                   attr(dset, "bracket")),
                              list("CDF showing what percentage of population accounts for how much of total national income",
                                   sum(dset[,attr(dset, "supp")]),
                                   "Total population"))
    title <- ggtitle(ds[[1]])
      switch(cumcdf,
             "bracket/supp" = ggtitle("CDF showing what percent of population is in each household income bracket in the United States (1999 dollars)"),
             "bracket/bracket*supp" = ggtitle("CDF showing what percent of total household income is accounted for by each income bracket in the United States (1999 dollars)"),
             "supp/bracket*supp" = ggtitle("CDF showing what percentage of population accounts for how much of total national income"))
    if (cumcdf != "supp/bracket*supp") {
      ulx <- attr(dset, "maxhh")
    } else {
      ulx <- sum(dset[,attr(dset, "supp")])
    }
    base <- ggplot(data.frame(x=c(0,attr(dset, "maxhh"))), aes(x)) + title +
      xlab(attr(dset, "bracket")) +
  ...
  main <- function(args=NULL) {
    dset <- doparser(name="looking",
                     ...
                     args)
    ggp.ecdf(dset, years=attr(dset, "years"), cumcdf=attr(dset, "cumcdf"))
    endup(dset)
    return(0)
  }
...
#+BEGIN_SRC R :tangle value.ecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
