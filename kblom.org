...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none
  ...
  rebin.binned <- function(vals, nobs, nvals) {
    require(iterators, quietly=TRUE)
    "given a set of bins (values), with its set of observations counts, produce a new set of bins, with a new set of observation counts.  the old values vals must fit \"integrally\" into the new vals.  returns the new observation counts."
    newcount <- vector()
    iter.nvals <- iter(nvals)
    nv <- nextElem(iter.nvals)
    iter.vals <- iter(vals)
    iter.nobs <- iter(nobs)
    v <- nextElem(iter.vals)
    count <- 0
    nnobs <- integer()
    tryCatch(
             repeat {
               ...
               while (v <= nv) {
                 count <- count + nextElem(iter.nobs)
                 v <- nextElem(iter.vals)
               }
             },
             error=function(e) if (e$message != "StopIteration") e,
             finally=nnobs <- c(nnobs, count))
    nnobs
  }
#+END_SRC
