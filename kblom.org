...
#+BEGIN_SRC R
  summary.binned <- function(vals, nobs) {
    ...
    } else if ((length(vals[!is.na(vals)]) == 0) &&
               (length(nobs[!is.na(nobs)]) == 0)) {
    ...
    xxx  colnames(m) <- c("value", "quantity")
    ## sort our input
    order <- order(vals)
    vals <- vals[order]
    nobs <- nobs[order]
