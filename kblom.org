...
85000 or less.  in 2016, .5 of pop made
...
#+BEGIN_SRC R :results none :eval no-export :exports none :var ifile=ifile
  ...
  doparser <- function(name, need, args) {
    ...
    if ("ifile" %in% need) {
      ...
      p <- add_argument(p, "--ifile", type="character", default=ifile,
...
#+BEGIN_SRC R :tangle create.totalincome.distribution :shebang "#!/usr/bin/env Rscript" :results none :eval no-export :exports none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
