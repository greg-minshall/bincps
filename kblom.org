...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  create.lookup <- function(dset, year, tgt.in.key) {
      ...
      tgtpct <- grepl("^%.*:.*$", tgt.in.key)
      keypct <- grepl("^.*:%.*$", tgt.in.key)
      tgt.in.key <- gsub("%", "", tgt.in.key)
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  ggp.ecdf <- function(dset, years, tgt.in.key, delta) {
    ds <- switch(tgt.in.key, 
                 ...
                 stop(sprintf("unknown \"--tgt.in.key\" argument \"%s\"", tgt.in.key)))
    title <- ds[[3]]
    if (delta) {
    ...
    if (delta) {
        ...
        maxy <- max(years)
        miny <- min(years)
        max.ecdf <- looking.create.ecdf(dset, maxy, tgt.in.key, count=FALSE)
        min.ecdf <- looking.create.ecdf(dset, miny, tgt.in.key, count=FALSE)
        base <- base + stat_function(fun=function(x) {
            max.ecdf(x)-min.ecdf(y)
        })
    } else {
        for (year in years) {
            ecdf <- looking.create.ecdf(dset, year=year, tgt.in.key=tgt.in.key, count=FALSE)
...
#+BEGIN_SRC R :tangle lookup :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  lookup <- function(dset, years, vals) {
      ...
      df <- data.frame()
      for (year in years) {
          ecdf <- create.lookup(dset, year=year, tgt.in.key=tgt.in.key, count=FALSE)
          yset <- dset[dset$YEAR==year,]
          for (val in vals) {
  ...
  main <- function(args=NULL) {
      dset <- doparser(name="lookup",
                       need=c("ifile",
                              ...
                              "years", "values"),
                       args)
      df <- lookup(dset, years=attr(dset, "years"), vals=attr(dset, "values"))
      endup(dset)
  }
