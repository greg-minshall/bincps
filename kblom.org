...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  doparser <- function(name, need, args) {
    ...
    if ("fourway" %in% need) {
        need <- subset(need, need!="fourway")
        p <- add_argument(p, "--fourway", type="boolean", flag=TRUE,
                          help="for lookup of tgt:key, return tgt:tgt OTHER(tgt):tgt key:tgt OTHER(key):tgt\n
  (where \"OTHER\" adds '%' if not already, otherwise removes the '%')")
    }      
...
#+BEGIN_SRC R :tangle lookup :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  lookup <- function(dset, years, tgt.in.key, keys, fourway) {
      df <- data.frame()
      for (year in years) {
