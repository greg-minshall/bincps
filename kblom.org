...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :results none :eval no-export :exports none
  ...
  ggp.ecdf <- function(dset, years, tgt.in.key, delta) {
    ...
    if (delta) {
        ...
        ## okay, in tgt.in.key, XXX assume key is "%", but that tgt is
        ## not
        sequence <- seq(from=0.00, to=1.00, by=0.01)
        deltas <- vapply(sequence, function(x) maxy.ecdf(x)-miny.ecdf(x),1)
        toplot <- data.frame(x=seq(0,1,.01), y=cumsum(deltas/sum(deltas)))
        base <- base + geom_smooth(data=toplot, aes(x=x,y=y),size=1, se=FALSE)
    } else {
        for (year in years) {
