...
in a [[https://twitter.com/kltblom/status/932394678241988609][tweet]], Kristian Blom showed [[file:./DPCIA2AUQAEO0lv.jpg][a histogram]] (1971-2015), from
Financial Times (based on data from Pew Trust).  Blom asked, given
that the past forty-plus years has seen *everyone* do better, should
we be worried about the growing inequality?
...
#+BEGIN_SRC R :session ss
  ...
  dset <- read.csv(ifile, header=TRUE)
  dset$YEAR <- as.factor(dset$YEAR)       # years are factors to us

  min <- min(dset$HHBRACKET99, na.rm=TRUE)
  max <- max(dset$HHBRACKET99, na.rm=TRUE)
  binsize <- 1000                         # XXX

  x <- seq(from=min, to=max, by=binsize)
  xyset <- data.frame()
  for (year in unique(dset$YEAR)) {
    sy <- dset$YEAR == year
    ecdf <- ecdf.binned(dset[sy,]$HHBRACKET99, dset[sy,supp])
    xyset <- rbind(xyset, data.frame(year=year, x=x, y=ecdf(x)))
  }
