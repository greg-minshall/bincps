  ...
  doparser <- function(name, need, args) {
    ...
    if ("keep" %in% need) {
      ...
      p <- add_argument(p, "--keep", type="numeric", default=10,
                        help="how many of top incomes to display")
    }
    if ("gopts" %in% need) {
...
XXX todo:
- XXX column (top and/or bottom) labels (from rownames)
- XXX separate identifier for getting palette right from labels (so we
  can say, "50,000 (10%)" for one year, and different percentage for a
  different year)
- title
- [[https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Box_Model/Introduction_to_the_CSS_box_model][CSS box model]] for the boxes: margin, border, padding, as if size
  were for entire cell (e.g., [[https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing][box-sizing]] [[https://css-tricks.com/inheriting-box-sizing-probably-slightly-better-best-practice/]["border box"]] model).
- so, also, color, size of border line (if any)
- allow user to inset figure a specified amount inside the graphic
  region (again, using CSS model-like)
- XXX figure out if the text fits in the boxes, maybe shrink the font
  size, or print a warning, if not.
...
#+begin_src R :exports none
  ...
  gsetup <- function(labels, title="",
                     titlefrac=0.33333,   # where title is horizontally
                     titledescent=1,      # title spaces above title
                     axisticascent=1      # axis tic mark spaces below axis tic mark
                     ) {
  ...
  a warning, if not."
      ...
      lines(c(0, xpx, xpx, 0, 0), c(0, 0, ypx, ypx, 0))

      ## size of labels in boxes, title, axis tic marks
      lsize <- gchmax(labels)
      tsize <- gchmax(title)
      asize <- gchmax(colnames(labels))

      ## where does title go?
      tlocx <- max(xpx*titlefrac, (tsize["width"]/2)+2) # in case title too
                                          # long
      if(tlocx+(tsize["width"]*.5) > xpx) { # make sure big enough
          stop(sprintf("title width (%d) too large for canvas (%d)",
                       tsize["width"], xpx))
      }
      tlocy <- ypx - (tsize["height"]*(titledescent+1))
      ## now, reserve this space by subtracting a bit more than this)
      if (ypx <= (tsize["height"]*(titledescent+2))) {
          stop(sprintf("title height (%d) * titledescent (%d) +2 > canvas height (%d)",
                       tsize["height"], titledescent, ypx))
      }
      ypx <- ypx - (tsize["height"]*(titledescent+2))

      ## where do axis tic marks go (in terms of Y)?
      alocy <- asize["height"]*axisticascent
      if (alocy >= ypx) {
          ## XXX we've confused things by decreasing ypx above...
          stop(sprintf("axis height (%d) * axisticascent (%d) >= room left in canvas height (%d)"))
      }
      ## and, reserve some room for this
      yoffpx <- alocy

      cellpx <- min((ypx-yoffpx)/nrow(labels), xpx/ncol(labels)) # size of a square
      xoffpx <- (xpx - (cellpx*ncol(labels)))/2 # x offset for lower left
      yoffpx <- yoffpx + (ypx - (cellpx*nrow(labels)))/2 # y ditto

      ## now, do labels fit?
      if ((lsize["height"] > cellpx) || (lsize["width"] > cellpx)) {
          stop(sprintf("at least one label is too large for box size"))
          ## too lazy to help the user...
      }
      ## what about axis tic marks?
      if (asize["width"] > cellpx) {
          stop(sprintf("at least one axis tic mark is too wide for box size"))
      }

      return(list(cellpx=cellpx,          # size of a square
                  xoffpx=xoffpx,          # bottom x offset
                  yoffpx=yoffpx,          # left y offset
                  tlocx=tlocx,            # x loc of title
                  tlocy=tlocy,            # y loc of title
                  alocy=alocy))           # y loc of axis tic marks
      }

  cboarddraw <- function(labels, background=NULL, title="") {
      "plots MxN LABELS, each with its own BACKGROUND color,
  with each label printing using common TEXTATTRIBUTES (font,
  size, style, color), in MxN boxes with common BOXATTRIBUTES
  (horizontal, vertical margins, border, padding, etc.)"

      stopifnot(compatiblep(background, labels))

      gp <- gsetup(labels=labels, title=title)
      ...
      txs <- lx+((ux-lx)/2)
      tys <- ly+((uy-ly)/2)
      ## now, for drawing tic marks
      justxs <- (((1:ncol(labels))-1)*cellpx)+xoffpx+(cellpx/2)
      ## draw text
      text(x=txs, y=tys, labels=as.vector(labels))
      text(x=gp[["tlocx"]], y=gp[["tlocy"]], labels=title)
      text(x=justxs, y=gp[["alocy"]], labels=colnames(labels))
  }

  rankboard <- function(labels, palette=NULL, ...) {
      "LABELS is presented sorted by rank; colnames are the time points.  PALETTE, if non-null, is the color palette to use (must be as long as the number of unique"

      labels <- as.matrix(labels)                       # data frame need not apply
      uni <- sort(unique(as.vector(labels)))           # number of unique labels
      ## (we sort uni so caller can maybe predict in case they care a
      ## lot about which color maps to which label value)
      if (is.null(palette)) {
          palette <- rainbow(length(uni), s=0.5)
      } else {
          stopifnot(length(palette) >= length(uni))
      }
      ## for match: http://r.789695.n4.nabble.com/Associative-array-tp1588978p1593950.html
      background <- matrix(palette[match(labels, uni)], nrow=nrow(labels))
...
get the data set of reported incomes
#+name: reported.dset
#+BEGIN_SRC R :results none :eval no-export :exports none :var ifile=ifile
  reported.dset <- function(argv) {        # command line args
    dset <- read.csv(argv$reported, header=TRUE)
    dset <- dset[dset$YEAR>=min(argv$years),]  # before 1967, NA data
...
#+BEGIN_SRC R :tangle bin/create.mostreported :shebang "#!/usr/bin/env Rscript" :results none :eval no-export :exports none
  ...
  mostreported <- function(rset, years, keep=30) {
      ...
      for (year in years) {
          ...
          fr <- yset$COUNT[order(yset$COUNT, decreasing=TRUE)]
...
** playing
*** [[https://stackoverflow.com/q/33886629/1527747][stackexchange answer]]                                           :noexport:
