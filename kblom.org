...
#+BEGIN_SRC R :session ss :results none
  ...
  hhbracket <- "HHBRACKET99"              # just for coding beauty
  binsize <- 1000                         # XXX
  LOWESTYEAR <- 1968                      # before 1968, NA data
  ...
  dset <- dset[dset$YEAR>=LOWESTYEAR,]     # before 1967, NA data

  min <- min(dset[, hhbracket], na.rm=TRUE)
  max <- max(dset[, hhbracket], na.rm=TRUE)

  ## create ECDF and inverse (IECDF)
  my.ecdf <- list()
  my.iecdf <- list()
  for (year in unique(dset$YEAR)) {
    sy <- dset$YEAR==year
    yset <- dset[dset$YEAR==year,]
    tidy <- tidyup(dset[sy,hhbracket], dset[sy,supp], MAXHH)
    ## now, get rid of outliers (tidyup included in 0, MAXHH indicies)
    dset <- dset[-which(sy & (dset[,hhbracket] < 0 | dset[,hhbracket] > MAXHH)),]
    sy <- dset$YEAR==year
    if (!all(dset[sy,hhbracket] == tidy$vals)) {
      stop(sprintf("internal error"))
    }
    dset[sy,supp] <- tidy$nobs
    my.ecdf[[year]] <- ecdf.binned(dset[sy, hhbracket], dset[sy, supp])
    my.iecdf[[year]] <- iecdf.binned(dset[sy, hhbracket], dset[sy, supp])
  }

  ggp.ecdf <- function(years=c(1973, 2015)) {
  ...
  ggp.tiles <- function(years=c(1972, 2014), tiles=c(0.1, 0.9)) {
  ...
  repro.chart <- function(dset,
    ...
    newvals <- rebinvals.binned(min(yset[,hhbracket], na.rm=TRUE),
                                max(yset[,hhbracket], na.rm=TRUE), binsize)
    dfh <- tidyup(nvals=newvals,
                  nobs=rebin.binned(vals=yset[,hhbracket], nobs=yset$HWTSUPP,
                    newvals=newvals), MAXHH)
