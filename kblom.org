...
#+begin_src R :exports none
  ...
  gchmax <- function(strings) {
   ...
   of space (in user coordinates) taken by the strings STRINGS"
      ...
      return(c(maxwidth=max(strwidth(strings)),
               maxheight=max(strheight(strings))))
  }
  ...
  a warning, if not."
      ...
      cellpx <- min(yoffpx/nrow(labels), xpx/ncol(labels)) # size of a square
      xoffpx <- (xpx - (cellpx*ncol(labels)))/2 # x offset for lower left
      yoffpx <- yoffpx + (ypx - (cellpx*nrow(labels)))/2 # y ditto

      ## now, does the text fit?  axis tic marks?  title?
      lmaxx <- max(strwidth(labels))
      lmaxy <- max(strheight(labels))


      return(list(cellpx=cellpx, 
                  xoffpx=xoffpx,
                  yoffpx=yoffpx,
                  lsize=lsize,
                  tsize=tsize,
