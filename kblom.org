...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  looking.create.ecdfs <- function(dset, cumcdf) {
    ...
    for (year in unique(dset$YEAR)) {
      ...
      my.ecdfs[[year]] <-
        switch(cumcdf,
               "population:bracket" = ecdf.binned(bracket, supp),
               "totalincome:bracket" = ecdf.binned(bracket, bracket*supp),
               ## in the following, we divide by "sum(supp)" to
               ## normalize across years (for comparability)
               "totalincome:population" = ecdf.binned(cumsum(supp)/sum(supp), bracket*supp))
    }
    my.ecdfs
  }
  ...
  looking.create.iecdfs <- function(dset, cumcdf) {
    ...
    for (year in unique(dset$YEAR)) {
      ...
      my.iecdfs[[year]] <-
        switch(cumcdf,
               "population:bracket" = iecdf.binned(bracket, supp),
               "totalincome:bracket" = iecdf.binned(bracket, bracket*supp),
               "totalincome:population" = iecdf.binned(cumsum(supp)/sum(supp), bracket*supp))
    }
    my.iecdfs
  }
...
#+BEGIN_SRC R :tangle values.ecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  values.ecdf <- function(dset, years, vals) {
      ...
      ecdfs <- looking.create.ecdfs(dset, cumcdf)
      df <- data.frame()
      for (year in years) {
          yset <- dset[dset$YEAR==year,]
          for (val in vals) {
              df <- rbind(df, data.frame(year=year,
                                         value=val,
                                         cumpct=ecdfs[[year]](val)))
          }
      }
      ccdfex <- cumcdf(cumcdf)
      colnames(df) <- c("year", ccdfex[3], ccdfex[2])
...
#+BEGIN_SRC R :tangle values.iecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  values.iecdf <- function(dset, years, tiles) {
    cumcdf <- attributes(dset)$cumcdf
    iecdfs <- looking.create.iecdfs(dset, cumcdf)
    df <- data.frame()
    for (year in years) {
      yset <- dset[dset$YEAR==year,]
      for (tile in tiles) {
        df <- rbind(df, data.frame(year=year, tile=tile, bracket=iecdfs[[year]](tile)))
      }
    }
    ccdfex <- cumcdf(cumcdf)
