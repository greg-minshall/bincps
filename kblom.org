...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none
  ...
  test.rebinvals.binned <- function() {
    "trivial unit test for rebinvals; built around
    for (i in c(-6:6, 24:29)) print(rebinvals(i:29, binsize=5))"
    testpat <- list(
...
#+BEGIN_SRC R :session ss :results none
  ...
  dset <- dset[dset$YEAR>1967,]        # before 1967, NA data
  ...
  repro.chart <- function(dset,
    ...
    tidyup <- function(yset, year) {
      nvals <- rebinvals(min(yset$HHBRACKET99, na.rm=TRUE),
                         max(yset$HHBRACKET99, na.rm=TRUE),
                         binsize)
      nobs <- rebin.binned(vals=yset$HHBRACKET99,
                           nobs=yset$HWTSUPP,
                           newvals=newvals)
      ## get rid of never-observed values (efficiency)
      nvals <- newvals[nobs!=0]
      nobs <- nobs[nobs!=0]
      tobs <- sum(nobs)               # sum over all (even high earners)
      ## now, clamp upper end at 200,000
      ## first, propagate counts of the higher incomes to end of chart
      highest <- max(nvals[nvals <= 200000], na.rm=TRUE)
      nobs[nvals==highest] <- sum(nobs[nvals>highest])
      ## now, delete the higher values
      nobs <- nobs[nvals >= 0 & nvals <= 200000]
      nvals <- nvals[nvals >= 0 & nvals <= 200000]
      ## data frame it
      df <- data.frame(year=as.factor(year), vals=nvals, nobs=nobs, tobs=tobs)
    }
    df <- tidyup(yset, hist)

    ## now, work on 1971
    yset <- dset[dset$YEAR==line,]
    df <- rbind(df, tidyup(yset, line))
    ...
    eval(base)                            # actually display

    ## by the way, the differences between the two curves should sum to
    ## (approx) zero
    sbzero <- sum((df[df$year==hist,"nobs"]/df[df$year==hist,"tobs"]) -
                  (df[df$year==line,"nobs"]/df[df$year==line,"tobs"]))
    if (abs(sbzero) > 0.001) {
