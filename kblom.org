...
#+BEGIN_SRC R :session ss :results none
  ...
  buildparser <- function(name, need) {
    ...
    if ("graphics" %in% need) {
      ...
      p <- add_argument(p, "--graphics", type="character", default="X11",
                        help="medium (\"X11\", \"pdf\", \"png\", etc.) to hold output graph")
    }
    if ("gfile" %in% need) {
    ...
    if ("years" %in% need) {
      ...
      p <- add_argument(p, "--years", type="character",
                        help="years to graph")
    }
    if ("tiles" %in% need) {
  ...
  dograph <- function(graphics, gfile, height, width, gopts) {
    "point our graphic output at a particular device (X11, pdf, png)"
    ...
    if (lgraphics == "x11") {
      ...
      if (!is.nacharacter(gfile)) {
        gopts <- topts("file", gfile)
      }
    } else {
      stop(sprintf("unknown --graphics option value \"%s\"; known values: \"x11\", \"pdf\", \"png\" (case independent)",
                   graphics))
    }
...
#+BEGIN_SRC R :tangle looking.ecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
...
#+BEGIN_SRC R :tangle looking.tiles :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
