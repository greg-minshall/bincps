...
#+BEGIN_SRC R :tangle bincps :shebang "#!/usr/bin/env Rscript" :session ss
  ...
  bincps <- function(ifile,      # input file
    ...
    if (nrow(rset) != 0) {                # anything to report?
      ...
      rset <- rset[order(rset$YEAR, rset[, income99]),]
...
#+BEGIN_SRC R
  check.binned <- function(fname, vals, nobs) {
    if (length(vals) == 0) {
      stop("summary.binned: no values")
    } else if (length(nobs) == 0) {
      stop("summary.binned: no observations")
    } else if (length(vals) != length(nobs)) {
      stop("summary.binned: length(values) != length(number of observations)")
    } else if (!is.numeric(vals[!is.na(vals)])) {
      stop("summary.binned: values must be numeric")
    } else if (!is.numeric(nobs[!is.na(nobs)])) {
      stop("summary.binned: number of observations must be numeric")
    }
  }
  ...
  ecdf.binned <- function(x, nobs) {
    ...
    vals <- unique(x)
    rval <- approxfun(vals, cumsum(tabulate.binned(match(x, vals), counts=nobs))/n,
                      method="constant",
                      yleft = 0, yright = 1,
                      f = 0, ties = "ordered")
  ...
  summary.binned <- function(vals, nobs) {
    summary.check("summary.binned", vals, nobs)
    ...
    ## computed by ecdf.  ecdf returns where, in **nobs**, the order
    ## lives.  then, we find the index of (approximately) that value,
    ## and return the *value* at that index
    result <- vapply(x,
                     function (e) { vals[nobs == floor(e)] },
                     FUN.VALUE=1)
