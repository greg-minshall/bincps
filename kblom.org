...
countably many) may have a positive probability of occurring, but the
sum of the probabilities over all those points will be less than one.
...
statistics guarantees (XXX) that over the long run, the empirical
distribution of a series of draws from a given distribution will
approach the theoretical distribution, it is also the case that, as
Keynes said, in the long run we will all be dead.  And, as it is often
the case, looking at real world "experiments", that we aren't really
certain of what theoretical distribution we are looking at, it is
sometimes more valuable to look at empirical distributions.

In addition to the probability distribution function, there's another
way of looking at a distribution, the "cumulative distribution
function" (CDF).  The CDF can never go down -- it can only go up (or
stay the same).  It is always in the range between zero and one; it
...
of the CDF at a given point is probability that the a sample taken
from the distribution will be less than or equal to that point.  Oof
-- what does that mean?
