...
#+BEGIN_SRC R :session ss :results none
  ...
  repro.chart <- function(dset) {
    yset <- dset[dset$YEAR==2016,]
    binsize <- 5000                       # what FT used
    nvals <- rebinlimits(min(yset$HHBRACKET99, na.rm=TRUE),
                         max(yset$HHBRACKET99, na.rm=TRUE),
                         binsize)
    nobs <- rebin.binned(vals=dset$HHBRACKET99, nobs=dset$HWTSUPP, newvals=newvals)
    ...
    ggplot(df, aes(vals, nobs/tobs))+geom_step()
  }
