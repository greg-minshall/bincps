...
#+BEGIN_SRC R :session ss :results none
  ...
  looking.create.ecdfs <- function(dset, cumcdf) {
    ## create ECDF
    my.ecdfs <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      ...
      my.ecdfs[[year]] <-
        switch(cumcdf,
               "bracket/supp" = ecdf.binned(bracket, supp),
               "bracket/bracket*supp" = ecdf.binned(bracket, bracket*supp),
               ## in the following, we divide by "sum(supp)" to
               ## normalize across years (for comparability)
               "supp/bracket*supp" = ecdf.binned(cumsum(supp)/sum(supp), bracket*supp))
    }
    my.ecdfs
  }

  looking.create.iecdfs <- function(dset, cumcdf) {
    ...
    for (year in unique(dset$YEAR)) {
      ...
      my.iecdfs[[year]] <-
        switch(cumcdf,
               "bracket/supp" = iecdf.binned(bracket, supp),
               "bracket/bracket*supp" = iecdf.binned(bracket, bracket*supp),
               "supp/bracket*supp" = iecdf.binned(cumsum(supp), bracket*supp))
    }
    my.iecdfs
  }
  ...
  doparser <- function(name, need, args) {
    ...
    if ("cumcdf" %in% need) {
      ...
      p <- add_argument(p, "--cumcdf", type="character", default="bracket/supp",
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
  ...
  ggp.ecdf <- function(dset, years, cumcdf, delta) {
    my.ecdfs <- looking.create.ecdfs(dset, cumcdf)
    ds <- switch(cumcdf, 
    ...
    if (delta) {
        ...
        base <- base + stat_function(fun=function(x) {
            y <- my.ecdfs[[maxy]](x)-my.ecdfs[[miny]](x)
            y
        })
    } else {
        for (y in years) {
...
#+BEGIN_SRC sh :tangle create.ecdf :shebang "#!/bin/sh" :results none :eval never
  ./create.ecdf.common $*
#+END_SRC

#+BEGIN_SRC sh :tangle create.ecdf2 :shebang "#!/bin/sh" :results none :eval never
  ./create.ecdf.common $* --maxhh 50000
#+END_SRC

**** create.tiles
#+BEGIN_SRC R :tangle create.tiles :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
...
#+BEGIN_SRC R :tangle create.repro :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
...
this returns 
#+BEGIN_SRC R :tangle values.ecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
  ...
  values.ecdf <- function(dset, years, vals) {
    ...
    for (year in years) {
      yset <- dset[dset$YEAR==year,]
      ecdf <- ecdf.binned(yset[,attr(dset, "bracket")],
                          yset[,attr(dset, "supp")])
      for (val in vals) {
...
this returns 
#+BEGIN_SRC R :tangle values.iecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
  ...
  values.iecdf <- function(dset, years, tiles) {
    ...
    for (year in years) {
      yset <- dset[dset$YEAR==year,]
      iecdf <- iecdf.binned(yset[,attr(dset, "bracket")],
                            yset[,attr(dset, "supp")])
      for (tile in tiles) {
...
#+BEGIN_SRC R :tangle create.totalincome.distribution :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
...
#+BEGIN_SRC R :tangle create.histo.hhbracket99 :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
...
#+BEGIN_SRC R :shebang "#!/usr/bin/env Rscript" :session ss :results none
  <<warning>>
  <<looking.bits>>
  <<stats.binned>>
