...
#+name: lucr
#+BEGIN_SRC R :exports none :var tgtinkey="bracket:%population" :var keys="0.5" :var years="1972,2016" :var ifile="ipums/cps_00006.1962-2017-binned.csv" :session ss
  <<lookup>>
  <<comprss>>

  df <- lookup.main(c("--ifile", ifile, "--years", years, "--tgt.in.key", tgtinkey, "--keys", keys))
  sprintf("$%s", comprss(df[,3]))
#+END_SRC
