...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none :eval no-export :exports none
  ...
  ecdf.binned.common <- function(x, nobs, inverse, from, cumsum) {
    ...
    if (cumsum) {                          # we want cumsum, not cumpct
...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  looking.create.ecdf <- function(dset, year, cumcdf, cumsum) {
      ## create ECDF
      sy <- dset$YEAR==year
      bracket <- dset[sy, attr(dset, "bracket")]
      supp <- dset[sy, attr(dset, "supp")]
      switch(cumcdf,
             "population:bracket" = ecdf.binned(bracket, supp, cumsum),
             "totalincome:bracket" = ecdf.binned(bracket, bracket*supp, sumcum),
             ## in the following, we divide by "sum(supp)" to
             ## normalize across years (for comparability)
             "totalincome:population" = ecdf.binned(cumsum(supp)/sum(supp), bracket*supp, cumsum))
  }

  looking.create.iecdf <- function(dset, year, cumcdf, cumsum) {
    ## create inverse (IECDF)
      sy <- dset$YEAR==year
      bracket <- dset[sy, attr(dset, "bracket")]
      supp <- dset[sy, attr(dset, "supp")]
      switch(cumcdf,
             "population:bracket" = iecdf.binned(bracket, supp, cumsum),
             "totalincome:bracket" = iecdf.binned(bracket, bracket*supp, cumsum),
             "totalincome:population" = iecdf.binned(cumsum(supp)/sum(supp), bracket*supp, cumsum))
  }
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  ggp.ecdf <- function(dset, years, cumcdf, delta) {
    ...
    if (delta) {
        ...
        maxy <- max(years)
        miny <- min(years)
        max.ecdf <- looking.create.ecdf(dset, maxy, cumcdf, cumsum=FALSE)
        min.ecdf <- looking.create.ecdf(dset, miny, cumcdf, cumsum=FALSE)
        base <- base + stat_function(fun=function(x) {
            max.ecdf(x)-min.ecdf(y)
        })
    } else {
        for (year in years) {
            ecdf <- looking.create.ecdf(dset, year=year, cumcdf=cumcdf, cumsum=FALSE)
...
#+BEGIN_SRC R :tangle create.tiles :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  ggp.tiles <- function(dset, years, tiles) {
    ...
    for (year in years) {
        iecdf <- looking.create.iecdf(dset, year=year, cumsum=FALSE)
...
#+BEGIN_SRC R :tangle values.iecdf :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  values.iecdf <- function(dset, years, tiles) {
    ...
    df <- data.frame()
    for (year in years) {
      iecdf <- looking.create.iecdf(dset, year=year, cumcdf=cumcdf)
      yset <- dset[dset$YEAR==year,]
      for (tile in tiles) {
