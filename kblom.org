...
#+BEGIN_SRC R :session ss :results none
  ...
  rebinvals <- function(min, max=NA, binsize) {
    "return the set of new values for a given resize"
    x <- min(c(min, max), na.rm=TRUE)
    y <- max(c(min, max), na.rm=TRUE)
    min <- x
    max <- y
    smin <- sign(min)
    smax <- sign(max)
    lo <- smin*(as.integer(abs(min)/binsize)*binsize)
    hi <- smax*(as.integer((abs(max)+binsize)/binsize)*binsize)
  ...
  repro.chart <- function(dset,
    ...
    tobs <- sum(df$nobs)
    base <- ggplot()
    base <- base + geom_step(data=df, aes(vals, nobs/tobs))

    ## now, work on 1971
    yset <- dset[dset$YEAR==line,]
    df <- tidyup(yset)
    tobs <- sum(df$nobs)
    base <- base + geom_line(data=df, aes(vals, nobs/tobs))
