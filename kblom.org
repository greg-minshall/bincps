...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none
  ...
  returns the new observation counts."
    ...
    if (vals[length(vals)] > newvals[length(newvals)]) { # by order(), s/b max
      ...
      stop(sprintf("rebin.binned: largest current bin (%d) greater than largest new bin (%d)",
                    vals[length(vals)], newvals[length(newvals)]))
    }
    j <- 1                                # index into newvals
    newnobs <- integer()                  # initialize return value
    count <- 0                            # intialize count (newnobs element)
    for (i in path) {
      while (vals[i] > newvals[j]) {      # test above guarantees eventual success
