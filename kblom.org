...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  part.desc <- function(part, pct) {
      ...
      if (pct) {
          rval <- sprintf("percent of %s", rval)
      }
      rval
  }

  part.max <- function(dset, years, part, pct) {
      if (pct) {
          return(1)                       # easy!
      }
      switch(part,
             "bracket" = attr(dset, "maxhh"),
             "totalincome" = sum(dset[dset$YEAR%in%years, attr(dset, "bracket")]*
                                dset[dset$YEAR%in%years, attr(dset, "supp")]),
             "population" = sum(dset[dset$YEAR%in%years, attr(dset, "supp")]))
  }x
  ...
  tgt.in.key <- function(tgt.in.key) {
      ...
      pcts <- grepl("^%.*$", parts)
      list(tgt=list(name=parts[1],
                pct=pcts[1],
                desc=part.desc(parts[1],pcts[1])),
           key=list(name=parts[2],
                pct=pcts[2],
                desc=part.desc(parts[2],pcts[2])))
  }
  ...
  create.lookup <- function(dset, year, tgt.in.key) {
      ...
      key <- cumpart(yset, parts[[1]]$name, parts[[1]]$pct)
      tgt <- cumpart(yset, parts[[2]]$name, parts[[2]]$pct)
