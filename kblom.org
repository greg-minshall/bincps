...
#+BEGIN_SRC R :session ss :tangle stats.binned :results none
  ...
  rebin.binned <- function(vals, nobs, newvals, ordered=FALSE) {
    require(iterators, quietly=TRUE)      # XXX could do without, but nice to have
  ...
  returns the new observation counts."
    ...
    if (length(vals) != length(nobs)) {
      error(sprintf("rebin.binned: length(vals) [%d] != length(nobs) [%d]",
                    length(vals), length(nobs)))
    }
    if (vals[length(vals)] > newvals[length(newvals)]) { # by order(), s/b max
      ## XXX could be a bit looser, only worry where nobs != 0
      error(sprintf("rebin.binned: largest current value (%d) greater than largest new value (%d)",
                    vals[length(vals)], newvals[length(newvals)]))
    }
    iter.newvals <- iter(newvals)
    nv <- nextElem(iter.newvals)
    count <- 0
    newnobs <- integer()
    for (i in path) {
      v <- vals[i]
      while (v > nv) {
        newnobs <- c(newnobs, count)
        count <- 0
        nv <- nextElem(iter.newvals)
      }
      count <- count + nobs[i]
    }
    newnobs <- c(newnobs, count)          # get last count
    ## fill out newnobs
    newnobs <- c(newnobs, rep(0, length(newvals)-length(newnobs)))
    newnobs                               # return value
  }
#+END_SRC
