...
#+BEGIN_SRC R :session ss :results none :eval no-export :exports none
  ...
  cumcdf <- function(cumcdf) {
      parts <- unlist(strsplit(cumcdf, ":")) # get bits of cumcdf
      if (length(parts) != 2) {
          stop(sprintf("unknown/improperly formatted cumcdf \"%s\"", cumcdf))
      }
      parts
  }
  ...
  cumpart <- function(dset, which, pct) {
  ...
  create.lookup <- function(dset, year, cumcdf) {
      "create a lookup function"
      yset <- dset[dset$YEAR==year,]
      ## first, remember and then get rid of percent signs
      tgtpct <- grepl("^%.*:.*$", cumcdf)
      keypct <- grepl("^.*:%.*$", cumcdf)
      cumcdf <- gsub("%", "", cumcdf)
      ## now, look at base key, target
      parts <- cumcdf(cumcdf)
      key <- cumpart(yset, parts[2], keypct)
      tgt <- cumpart(yset, parts[1], tgtpct)
...
#+BEGIN_SRC R :tangle lookup :shebang "#!/usr/bin/env Rscript" :session ss :results none :eval no-export :exports none
  ...
  lookup <- function(dset, years, vals) {
      ...
      df <- data.frame()
      for (year in years) {
          ecdf <- create.lookup(dset, year=year, cumcdf=cumcdf, count=FALSE)
          yset <- dset[dset$YEAR==year,]
          for (val in vals) {
