...
#+BEGIN_SRC R :session ss :results none
  ...
  tidyup <- function(nvals, nobs, maxval) {
    ...
    if (!is.numeric(highest)) {
      stop(sprintf("tidyup: all values greater than maxval (%g)", maxval))
    }
    ...
    if (!is.numeric(lowest)) {
      stop(sprintf("tidyup: all values less than maxval (%g) also less than zero",
                   maxval))
    }
  ...
  looking.dset <- function(ifile,         # input file name
                           maxhh=200000,
                           supp="HWTSUPP", # which variant to use
                           bracket="HHBRACKET99", # just for coding beauty
                           binsize=1000,          # XXX
                           fyear=1968) { # before 1968, NA data
    dset <- read.csv(ifile, header=TRUE)
    dset <- dset[dset$HFLAG == FALSE | is.na(dset$HFLAG),]
    dset <- dset[dset$YEAR>=fyear,]       # before 1967, NA data

    min <- min(dset[, bracket], na.rm=TRUE)
    max <- max(dset[, bracket], na.rm=TRUE)

    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      yset <- dset[dset$YEAR==year,]
      tidy <- tidyup(dset[sy,bracket], dset[sy,supp], maxhh)
      ## now, get rid of outliers (tidyup included in 0, maxhh indicies)
      dset <- dset[-which(sy & (dset[,bracket] < 0 | dset[,bracket] > maxhh)),]
      sy <- dset$YEAR==year
      if (!all(dset[sy,bracket] == tidy$vals)) {
        stop(sprintf("internal error"))
      }
      dset[sy,supp] <- tidy$nobs
    }
    ## now, remember parameters in attributes
    attr(dset, "maxhh") <- maxhh
    attr(dset, "supp") <- supp
    attr(dset, "bracket") <- bracket
    attr(dset, "binsize") <- binsize
    attr(dset, "fyear") <- fyear
    dset
  }

  looking.ecdfs <- function(dset) {
    ## create ECDF
    my.ecdf <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      my.ecdf[[year]] <- ecdf.binned(dset[sy, attr(dset, "bracket")],
                                     dset[sy, attr(dset, "supp")])
    }
    my.ecdf
  }

  looking.iecdfs <- function(dset) {
    ## create inverse (IECDF)
    my.iecdf <- list()
    for (year in unique(dset$YEAR)) {
      sy <- dset$YEAR==year
      my.iecdf[[year]] <- iecdf.binned(dset[sy, attr(dset, "bracket")],
                                       dset[sy, attr(dset, "supp")])
    }
    my.iecdf
  }
  ...
  ggp.ecdf <- function(dset, years=c(1973, 2015)) {
    myecdfs <- looking.ecdfs(dset)
    base <- ggplot(data.frame(x=c(0,maxhh)), aes(x))
    for (y in years) {
  ...
  ggp.tiles <- function(dset, years=c(1972, 2014), tiles=c(0.1, 0.9)) {
    ...
    myiecdfs <- looking.iecdfs(dset)
    base <- ggplot()
    for (y in years) {
      df <- data.frame(year=as.factor(y),
                       tile=tiles,
                       hhincome99=my.iecdfs[[y]](tiles))
      base <- base + geom_point(data=df, aes(x=tile, y=hhincome99, colour=year))
    }
    print(base)
  }
  ...
  pyear <- function(iecdf, y1, y2, tiles) {
    ...
    my1 <- min(y1, y2)
    my2 <- max(y1, y2)
    (iecdf[[y2]](tiles)/my.iecdf[[y1]](tiles))^(1/(y2-y1))-1
  }
  ...
  main <- function(args=NULL) {
    ...
    p <- arg_parser("looking")
    p <- arg_parser(p, "--ifile", type="character", default=NA,
    ...
    looking.dset(ifile=argv$ifile,
                         maxhh=argv$maxhh,
                         supp=argv$supp,
                         bracket=argv$bracket,
                         binsize=argv$binsize,
                         fyear=argv$fyear)
  }
