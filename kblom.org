...
#+BEGIN_SRC R :session ss :results none
  ...
  doyears <- function(stryears) {
    assert_that(!is.na(stryears), msg="required parameter --years missing")
    as.numeric(unlist(strsplit(stryears, "[ ,]+")))
  }

  dotiles <- function(strtiles) {
    assert_that(!is.na(strtiles), msg="required parameter --tiles missing")
    as.numeric(unlist(strsplit(strtiles, "[ ,]+")))
  }
  ...
  undograph <- function(graphics, gopts) {
...
#+BEGIN_SRC R :tangle create.ecdf.common :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  main <- function(args=NULL) {
    ...
    dograph(graphics=argv$graphics,
            ...
            width=argv$width,
            gopts=argv$gopts)
    ggp.ecdf(dset, years=argv$years)
    undograph(argv$graphics, argv$gopts)
    return(0)
  }
...
#+BEGIN_SRC R :tangle create.tiles :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  main <- function(args=NULL) {
    ...
    dograph(graphics=argv$graphics,
            ...
            width=argv$width,
            gopts=argv$gopts)
    ggp.tiles(dset, years=argv$years, tiles=argv$tiles)
    undograph(argv$graphics, argv$gopts)
    return(0)
  }
...
#+BEGIN_SRC R :tangle create.repro :shebang "#!/usr/bin/env Rscript" :session ss :results none
  ...
  main <- function(args=NULL) {
    ...
    dograph(graphics=argv$graphics,
            ...
            width=argv$width,
            gopts=argv$gopts)
    repro.chart(dset, bracket=argv$bracket, supp=argv$supp, years=argv$years)
    undograph(argv$graphics, argv$gopts)
    return(0)
  }
